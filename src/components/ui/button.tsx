  ...
  "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-lg text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0 ",
  {
    variants: {
      variant: {
        ...
        outline:
          "border text-brand-primary border-brand-primary border-input bg-background shadow-sm hover:bg-accent hover:text-accent-foreground hover:bg-brand-primary hover:text-white",
        secondary:
...
export interface ButtonProps
  ...
  asChild?: boolean
  loading?: boolean
  loaderColor?: "default" | "white" | "primary" | "secondary"
}

const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, asChild = false, loading = false, loaderColor = "white", children, disabled, ...props }, ref) => {
    ...
    return (
      ...
      >
        {children}
        {loading && <Spinner size="sm"  color={loaderColor} />}
      </Comp>
    )
  }
)
